    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <!-- Form -->
                <form action="{{ route('salesReturns.update', 'test') }}" method="post" enctype="multipart/form-data">
                    {{ method_field('patch') }}
                    @csrf
                    <div class="row">
                        <!-- date -->
                        ...
                        <!-- client_id -->
                        <div class="col-4">
                            <label for="client_id" class="mr-sm-2">{{ trans('main.Client') }} :</label>
                        ...
                        <div class="col-12 mt-3" style="border-top: 1px solid grey;">
                            <div id="purchase_details" name="purchase_details">
                                ...
                                <table id="myTable" class="col-12">
                                    @foreach($item->details as $sale_detail)
                                    <tr>
                                        <td style="width:48%;">
                                            <label for="product_id" class="mr-sm-2">{{ trans('main.Product') }} :</label>
                                            <select class="form-control select2" name="product_id[]" required>
                                                <option label="{{ trans('main.Choose') }}"></option>
                                                <option value="{{$sale_detail->product_id}}">{{$sale_detail->product->name}}</option>
                                            </select>
                                        </td>
                                        <td style="width:3%;"></td>
                                        <td style="width:48%;">
                                            <label for="quantity" class="mr-sm-2">{{ trans('main.Quantity') }} :</label>
                                            <input id="quantity" type="number" class="form-control" name="quantity[]" value="{{ $sale_detail->quantity }}" required>
                                        </td>
                                        <td style="width:1%;"></td>
                                    </tr>
                                    @endforeach
