    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ trans('main.Adjustments') }}</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
...
@section('content')
        ...
        <div class="row">
                    ...
                    <div id="print">
                        <div class="card-header pb-0">
                            ...
                            <div class="d-flex justify-content-between">
                                <h4 class="card-title mg-b-0">{{ trans('main.Adjustments') }}</h4>
                            ...
                            <div class="row row-xs">
                                <div class="col-sm-6 col-md-12 notPrint">
                                    @if($trashed == false)
                                        ...
                                        @endcan
                                        <!-- <a class="btn btn-danger ripple float-left" href="{{ route('adjustments.archived') }}">
                                            <i class="fe fe-alert-triangle"></i>&nbsp;{{ trans('main.Archive') }} {{ trans('main.Adjustments') }}
                                        </a> -->
                                    @else
                                        @can('عرض تعديل المخزون')
                                            <a class="btn btn-danger ripple float-left" href="{{ route('adjustments.index') }}">
                                    ...
                                    <tbody>
                                        ...
                                        @if($trashed == false)
                                            <tr class="notPrint">
                                                ...
                                                <td class="text-center"></td>
                                                <form action="{{ route('adjustments.index') }}" method="get">
                                                    <td class="text-center">
                                                        <input type="date" class="form-control text-center" name="date" placeholder="{{ trans('main.Date') }}"  value="{{ $date }}">
                                                    </td>
                                                    <td class="text-center">
                                                    ...
                                                    <td class="text-center">
                                                        <select class="form-control text-center" name="type">
                                                            <option value="" selected>{{ trans('main.All') }}</option>
                                                            <option value="add" {{$type == 'add' ? 'selected' : ''}}>{{ trans('main.Add') }}</option>
                                                            <option value="subtract" {{$type == 'subtract' ? 'selected' : ''}}>{{ trans('main.Subtract') }}</option>
                                                        </select>
                                                    </td>
                                                    <td class="text-center">
                                                        <input type="number" class="form-control text-center" name="quantity" placeholder="{{ trans('main.Quantity') }}" value="{{ $quantity }}">
                                                    </td>
                                                    <td class="text-center">
                                        ...
                                        @foreach ($adjustments as $item)
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center">{{ $item->details[0]->product->name }}</td>
                                                <td class="text-center">{{ $item->details[0]->type == 'add' ?  'إضافة' : 'نقص' }}</td>
                                                <td class="text-center">{{ $item->details[0]->quantity }}</td>
                                                <td class="text-center notPrint">
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu tx-13 bd-primary rounded-5">
                                                            @if($trashed == false)
                                                                @can('عرض تعديل المخزون')
                                                                    <a class="dropdown-item" href="{{ route('adjustments.show',[$item->id]) }}" title="{{ trans('main.Show') }}">
                                            ...
                                            @include('dashboard.adjustment.editModal')
                                            @include('dashboard.adjustment.deleteModal')
                                            @include('dashboard.adjustment.forceDeleteModal')
                                            @include('dashboard.adjustment.restoreModal')
            ...
            @include('dashboard.adjustment.addModal')
            @include('dashboard.adjustment.deleteSelectedModal')
...
@section('js')
    ...
    <script type="text/javascript">
        function checkQuantity()
        {
            ...
            if(quantity < 0)
            {
                alert('يجب أن تكون الكمية أكبر من 0');
