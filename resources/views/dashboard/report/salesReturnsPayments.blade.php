...
@section('css')
    @section('title')
    {{ trans('main.Report') }} {{ trans('main.payments') }} {{ trans('main.Returns') }} {{ trans('main.Sales') }}
    @stop
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ trans('main.Report') }} {{ trans('main.payments') }} {{ trans('main.Returns') }} {{ trans('main.Sales') }}</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
                    ...
                    <div id="print">
                        <div class="card-header pb-0">
                            ...
                            <div class="d-flex justify-content-between">
                                <h4 class="card-title mg-b-0">{{ trans('main.Report') }} {{ trans('main.payments') }} {{ trans('main.Returns') }} {{ trans('main.Sales') }}</h4>
                            ...
                            <!-- start search -->
                            <form action="{{ route('reports.salesReturnsPayments') }}" method="get">
                                <div class="row mt-5">
                                    ...
                                    <div class="col-2 mr-2 notPrint">
                                        <label for="to_date" class="">{{ trans('main.From Date') }} :</label>
                        ...
                        <div class="card-body">
                            <div class="table-responsive hoverable-table">
                                <table class="table table-striped" id="example" data-page-length='50' style=" text-align: center;">      
                                    ...
                                    <tbody>
                                        <?php $i = 0; ?>
                                        @foreach($salesReturnsPayments as $item)
                                            <?php $i++; ?>
                                            <tr>
                                                <td class="text-center">{{ $i }}</td>
                                                <td class="text-center">{{ $item->date }}</td>
                                                <td class="text-center">{{ $item->Ref }}</td>
                                                <td class="text-center">{{ $item->SaleReturn->Ref }}</td>
                                                <td class="text-center">{{ $item->SaleReturn->client->name }}</td>
                                                <td class="text-center">{{ $item->montant }}</td>
                                                <td class="text-center">{{ $item->SaleReturn->warehouse->name }}</td>
